...
- (BOOL)PushIntoStack:(NSString *)str {
    ...
    if (self.array.count == 0 || [self.operatorArray indexOfObject:str]/2 > [self.operatorArray indexOfObject:self.array[self.array.count - 1]]/2 || [str isEqual:@"("] || [self.array.lastObject isEqual:@"("]) {
        //栈为空或者操作符的优先级比栈顶元素高
...
- (BOOL)isHigh:(NSString *)chars {
    if ([self.operatorArray indexOfObject:chars]/2 > [self.operatorArray indexOfObject:self.array[self.array.count - 1]]/2 || [self.array.lastObject isEqual:@"("]) {
        return YES;
    }
    return NO;
}

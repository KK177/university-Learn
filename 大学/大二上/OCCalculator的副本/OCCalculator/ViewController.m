...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!text.length) {
        ...
        BOOL flag = ([Chars isEqual:@"÷"] || [Chars isEqual:@"×"] ||[Chars isEqual:@"+"] || [Chars isEqual:@"-"]||[Chars isEqual:@"="] || [Chars isEqual:@"."] || [Chars isEqual:@"～"] || [Chars isEqual:@"C"] || [Chars isEqual:@"←"]);
        if (flag) {
            //第一个是操作符不做任何处理
        }else {
            //第一个不是操作符
...
- (void) TranslateToInversePolandForClculate:(NSMutableArray *)array {
    ...
    for (int i=0; i<array.count; i++) {
        ...
        //如果是操作符
        if (flag) {
            while ([Chars isEqual:@"/"] || [Chars isEqual:@"*"] ||[Chars isEqual:@"+"] || [Chars isEqual:@"-"]) {
                //先判断是否入栈成功
